  ...
  class TrakerrClient
    ...
    def initialize(api_key,
      ...
      when /mswin|msys|mingw|cygwin|bccwin|wince|emc/
        ...
        @context_app_os.chomp! unless @context_app_os.nil?
        @context_app_os.strip! unless @context_app_os.nil?

        version = get_text_from_line(text, 'OS Version:', "\n").split
        version[0].chomp! unless version.nil?
        version[0].strip! unless version.nil?
      ...
      @context_app_os = RbConfig::CONFIG['target_os'] if @context_app_os.nil?
    ...
    def log(arg_hash, error, log_level = 'error', classification = 'issue')
      ...
      if error != false
        ...
        app_event = create_app_event(error, log_level, classification, arg_hash.fetch('eventname', 'unknown'), arg_hash.fetch('evntmessage', 'unknown'))

      end
      app_event = create_app_event(false, log_level, classification, arg_hash.fetch('eventname', 'unknown'), arg_hash.fetch('evntmessage', 'unknown')) if app_event.nil?
      app_event.event_user = arg_hash['user'] if arg_hash.key? 'user'
      app_event.event_session = arg_hash['session'] if arg_hash.key? 'session'
